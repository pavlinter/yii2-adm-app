    ...
    if (!Yii::$app->user->isGuest) {
        ...
        $view->on($view::EVENT_END_BODY, function ($event) {
            ...
            <li class="media {{? !it.viewed }}active{{?}}" id="{{=it.unique}}">
                ...
                <a href="{{=it.href || 'javascript:void(0);'}}" class="link-block clearfix">
                    <div class="media-left relative">
                        {{? it.icon }}
                            {{=it.icon}}
                        {{??}}
                            {{=it.inlineHtml || ''}}<img src="{{=it.src}}" class="media-object" alt="" />
                        {{?}}
                    </div>
                    <div class="notifi-media-body">
                        <h6 class="media-heading">{{=it.header || ''}}</h6>
                        <p>{{=it.msg}}</p>
                        <div class="notifi-time">{{=it.date}}</div>
                    </div>
                </a>
            </li>
            <?php DoT::end(); ?>
