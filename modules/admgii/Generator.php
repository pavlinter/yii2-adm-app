...
abstract class Generator extends Model
{
     ...
     * Please refer to [[\app\modules\admgii\generators\controller\Generator::generate()]] as an example
     ...
     * @return string
     */
    public function generateString($string = '', $placeholders = [], $subcategory = '')
    {
        $string = addslashes($string);
        $messageCategory = $this->messageCategory;
        if($subcategory){
            $messageCategory = $this->messageCategory . '/' . $subcategory;
        }
        if ($this->enableI18N) {
